	...
	<div class="container mx-auto p-4">
		...
		<div class="mb-6 rounded-lg border border-slate-700 bg-slate-800 p-4">
			<div class="flex flex-wrap items-end gap-4">
				<div class="flex-grow">
					<label for="searchTerm" class="mb-1 block text-sm font-medium text-slate-300">
						Search Resources
					</label>
					<input
						type="text"
						...
						class="w-full rounded-md border border-slate-600 bg-slate-700 px-3 py-2 text-white placeholder-slate-400 focus:border-blue-400 focus:ring-1 focus:ring-blue-400 focus:outline-none"
					/>
				</div>
			...
			<div class="mt-4 flex items-center">
				<label class="flex items-center">
					<input
						...
						class="rounded border-slate-600 bg-slate-700 text-blue-600 focus:ring-blue-400"
					/>
		...
		{#if resourceStats.total > 0}
			<div class="mb-6 rounded-lg border border-slate-700 bg-slate-800 p-4">
				<h2 class="mb-3 text-lg font-semibold text-white">Resource Statistics</h2>
				<div class="grid grid-cols-3 gap-4 text-center">
					<div class="rounded-lg bg-blue-900/30 p-3">
						<div class="text-2xl font-bold text-blue-400">
							{resourceStats.total}
						</div>
						<div class="text-sm text-blue-300">Total Resources</div>
					</div>
					<div class="rounded-lg bg-green-900/30 p-3">
						<div class="text-2xl font-bold text-green-400">
							{resourceStats.active}
						</div>
						<div class="text-sm text-green-300">Currently Spawned</div>
					</div>
					<div class="rounded-lg bg-red-900/30 p-3">
						<div class="text-2xl font-bold text-red-400">
							{resourceStats.despawned}
						</div>
						<div class="text-sm text-red-300">Despawned</div>
					</div>
				</div>
			</div>
		{/if}
		...
		{#if enhancedSearch && soapResults.length > 0}
			<div class="mb-6 rounded-lg border border-yellow-600 bg-yellow-900/20 p-4">
				<h2 class="mb-3 text-lg font-semibold text-yellow-200">
				...
				<div class="mt-2 space-y-1">
					{#each soapResults as soapResult}
						<div class="rounded bg-yellow-800/30 px-2 py-1 text-sm text-yellow-200">
							{soapResult}
						</div>
					{/each}
				</div>
		...
		<div class="mb-4 flex justify-end">
			<div class="inline-flex rounded-md">
				<button
					type="button"
					class="rounded-l-lg border border-slate-600 px-4 py-2 text-sm font-medium text-white {viewMode ===
					'table'
						? 'bg-blue-600'
				...
				<button
					type="button"
					class="rounded-r-lg border border-slate-600 px-4 py-2 text-sm font-medium text-white {viewMode ===
					'grid'
						? 'bg-blue-600'
						: 'bg-slate-800 hover:bg-slate-700'}"
		...
		{#if viewMode === 'table'}
			<div class="overflow-hidden rounded-lg border border-slate-700 bg-slate-800">
					...
					<table class="min-w-full divide-y divide-slate-600">
						...
						<tbody class="divide-y divide-slate-600 bg-slate-800">
							{#each resources as resource}
								<tr
									class="cursor-pointer hover:bg-slate-700"
								...
								>
									...
									<td class="px-4 py-2 text-sm whitespace-nowrap text-slate-400">
				...
				{#if resources.length > 0}
					<div class="bg-slate-700 px-4 py-3 text-sm text-slate-300">
		...
		{:else}
			...
			<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
				...
				{:else}
					<div class="col-span-full p-4 text-center text-slate-400">
						{isLoading ? 'Loading resources...' : 'No resources found'}
					</div>
				{/each}
			</div>
